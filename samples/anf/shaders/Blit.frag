...
//-----------------------------------------------------------------------------
{
    ...
    // MV Visualization (if DiffuseColor == MV)
#if 0
    vec2 velPixels = DiffuseColor.xy;
    vec2 velNorm   = velPixels / 1.0;
    vec2 vis       = velNorm * 0.5 + 0.5;
    FragColor      = vec4(vis, 0.0, 1.0);

    float mag = length(DiffuseColor.xy) / 10.0;
