...
{
    mat4   CurrViewProj;         // Jittered view-projection matrix for current frame (raster)
    mat4   PrevViewProj;         // (unused)
    mat4   CurrViewProjNoJitter; // Non-jittered VP for current frame (motion vectors)
    mat4   PrevViewProjNoJitter; // Non-jittered VP for previous frame (motion vectors)
    mat4   ModelMatrix;
...
layout (location = 7) out vec4    v_CurrClip; // No-jitter current clip (MV)
layout (location = 8) out vec4    v_PrevClip; // No-jitter prev clip (MV)
...
void main()
{
    ...
    // World positions for current and previous frame (static objects usually identical)
    ...
    // Raster path (jittered)
    gl_Position = VertCB.CurrViewProj * currWorldPos4;

    // Motion vector path (non-jittered)
    ...
    // Shadows (uses current world position)
