...
//============================================================================================================
//
// Mode 3 VK-CL interop implementation (InteropImageOptimal):
//   - Shares VkImage (optimal tiling) with OpenCL via QCOM Vulkan opaque fd extension
...
//
//   Note:
//   - CL_EXTERNAL_MEMORY_HANDLE_VULKAN_OPAQUE_FD_QCOM can also be used for
//     linear-tiling image interop with the same flow used in this file.
//   - It can also be used for buffer interop, where its usage is the same as
//     CL_EXTERNAL_MEMORY_HANDLE_OPAQUE_FD_KHR for buffers.
//
//============================================================================================================
...
// ---------------------------------------------------------------------------
InteropImageOptimal::Context::Context()
{
}

// ---------------------------------------------------------------------------
bool InteropImageOptimal::Context::Initialize(
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::Release(GraphicsApiBase& gfxApi)
{
    auto& vulkan    = static_cast<Vulkan&>(gfxApi);
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::Dispatch()
{
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::CopyToSharedImage(
    Vulkan&            vulkan,
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::CopyFromSharedImage(
    Vulkan&            vulkan,
    CommandListVulkan& command_list)
{
    VkCommandBuffer cmd           = command_list.m_VkCommandBuffer;
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::ReleaseSharedResourcesToExternal(
    Vulkan&            vulkan,
    CommandListVulkan& command_list)
{
    VkCommandBuffer cmd = command_list.m_VkCommandBuffer;
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::AcquireSharedResourcesFromExternal(
    Vulkan&            vulkan,
    CommandListVulkan& command_list)
{
    VkCommandBuffer cmd = command_list.m_VkCommandBuffer;
...
// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::InitSharedImage(
    GraphicsApiBase&  gfxApi,
    ...
    cl_mem_flags      mem_flags)
{
    ...
    image_info.tiling        = VK_IMAGE_TILING_OPTIMAL;  // ← optimal tiling
    ...
    const bool supports_linear_filtering =
        SupportsLinearFiltering(vulkan.m_VulkanGpu, format, VK_IMAGE_TILING_OPTIMAL);
    ...
    std::vector<cl_mem_properties> props{
        (cl_mem_properties)CL_EXTERNAL_MEMORY_HANDLE_VULKAN_OPAQUE_FD_QCOM,
    ...
    cl_desc.image_row_pitch   = 0; // must be 0 for CL_EXTERNAL_MEMORY_HANDLE_VULKAN_OPAQUE_FD_QCOM
    cl_desc.image_slice_pitch = 0; // must be 0 for CL_EXTERNAL_MEMORY_HANDLE_VULKAN_OPAQUE_FD_QCOM
    ...
    shared_image.cl_image = clCreateImageWithProperties(
        ...
        nullptr, &cl_desc, nullptr, &err);
    CL_CHECK(err);
}

// ---------------------------------------------------------------------------
void InteropImageOptimal::Context::InitCLKernel(AssetManager& assetManager)
{
    LOGI("*************************************");
